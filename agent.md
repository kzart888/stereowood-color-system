...
- Near-term priorities:
  1. Phase C：交付配方输入组件（芯片/建议）并集成 SchemeDialog。
  2. Phase D：实现候选色列表与留空流程，联通 FormulaMatcher ΔE 逻辑。
  3. Phase E：完善作品/字典视图的重复 & 异常标记，补齐单元 + Playwright 验证。
  4. 更新部署文档，涵盖 `manual_formula` 迁移及后续 Phase F API 扩展计划。
